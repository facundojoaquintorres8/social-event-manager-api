...
public class ContributionController {
    ...
    @PatchMapping("/{contributionId}/status")
    public ResponseEntity<ApiResponseDTO<Void>> updateContributionStatus(
            @PathVariable UUID eventId,
            @PathVariable UUID contributionId,
            @Valid @RequestBody UpdateContributionStatusRequestDTO request) {
        return ResponseEntity.ok(
                contributionService.updateContributionStatus(
                        eventId,
                        contributionId,
                        request));
    }
